...
var questionType = "";
var userDataEstate = document.getElementById("userDataEstate");
var xmlHttp=new XMLHttpRequest();

function presentData()
{
    changeVisibility("userDataPresenter","block");
    xmlHttp.onreadystatechange = function(){
        if(this.readyState == 4 && this.status == 200){
            document.getElementById("userDataEstate").innerHTML=this.responseText;
        }
    };

    xmlHttp.open("GET","../scripts/getparticularuser.php?u="+document.getElementById("usernameEntry").value,true);
    xmlHttp.send();
...
function onViewSelected(){
    ...
    else if (user.checked)
    {
        ...
        changeVisibility("usernameDeleteEntry","block");
